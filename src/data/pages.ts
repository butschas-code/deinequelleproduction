/**
 * Vollständiger Textbestand aus der bisherigen Website deinequelle.com (abgeglichen mit Live-HTML).
 * Rechtstexte: zusätzlich vollständig als PDF unter /documents und als Plaintext unter src/content/legal.
 */
export const pages = {
  home: {
    title: "DEINE QUELLE – Mehr Leben",
    heroEyebrow: "Herzlich willkommen",
    heroTitle: "Schön, bist du hier!",
    intro:
      "Ich freue mich, dich mit meinen Angeboten\nin Yoga und Kinesiologie ein Stück\nauf deinem Lebensweg zu begleiten.",
    yogaTeaser: {
      heading: "Hatha Yoga mit Pratyahara-Meditation",
      body: "Die Harmonisierung von Körper, Geist und Seele durch sanfte Yogaübungen, bewusste Atemtechniken und Yoga Nidra als wohltuende Form der Tiefenentspannung und inneren Regeneration.",
      linkLabel: "Mehr Informationen",
    },
    kinesiologieTeaser: {
      heading: "Komplementärtherapie — Methode Kinesiologie",
      body: "Durch gezielte Therapieeinheiten wird die physische, mentale und emotionale Balance wiederhergestellt und das innere Gleichgewicht nachhaltig gestärkt.",
      linkLabel: "Mehr Informationen",
    },
  },
  kinesiologie: {
    title: "KINESIOLOGIE – DEINE QUELLE",
    h1: "KINESIOLOGIE",
    sub: "Komplementärtherapie",
    introTitle: "KINESIOLOGIE - DIE LEHRE DER BEWEGUNG",
    intro:
      "Kinesiologie ist eine anerkannte Methode der Komplementärtherapie. Sie unterstützt Menschen darin, das Wohlbefinden zu erhöhen, die mentalen Fähigkeiten zu stärken und ganzheitlich zu genesen. Die Methode verbindet Erkenntnisse aus der westlichen Medizin mit dem Wissen fernöstlicher Lehren, wie jene der fünf Elemente oder der Lehre der Meridiane. Dabei nutzt die Methode im therapeutischen Prozess die Beziehung der Muskeln zum menschlichen Organismus, um Ungleichgewichte im Energiesystem zu erkennen und Veränderungen erlebbar zu machen.",
    sportBlock: {
      title: "SPORT-KINESIOLOGIE",
      body: "Sportkinesiologie steigert die Leistungsfähigkeit von Sportlerinnen und Sportlern, indem sie Ungleichgewichte in Biomechanik, Biochemie und Psyche ausgleicht und so das volle Potenzial entfaltet.",
      linkLabel: "Mehr Information zur Sport-Kinesiologie",
    },
    wannHeading: "Wann wird Kinesiologie angewendet?",
    wann:
      "Die komplementärtherapeutische Methode Kinesiologie eignet sich für Kinder, Jugendliche, Erwachsene und ältere Menschen in den unterschiedlichsten Lebenssituationen und unterstützt sie darin, das Wohlbefinden zu erhöhen, die mentalen Fähigkeiten zu stärken und ganzheitlich zu genesen.",
    wieHeading: "Wie wirkt Kinesiologie?",
    wie: "Durch den Ausgleich der Energie über die Meridiane erhöht und stärkt Kinesiologie die selbstregulierenden Kräfte des Menschen. In der therapeutischen Praxis zeigt sich: Die Anleitung zur Körperwahrnehmung und der Möglichkeit, Reaktionen des Organismus über die Muskelsignale zu erkennen sowie die Veränderung mithilfe von einem Vorher-Nachher-Erlebnis zu erfahren, motiviert die Klient*innen, ihren Körper in Alltagssituationen zu beobachten und auf seine Signale zu achten. Nebst der Verbesserung der Selbstwahrnehmung können durch die Reflexion mit dem Therapierenden körperliche, seelische und geistige Ressourcen evaluiert und diese gezielt zur Förderung der Gesundheit eingesetzt werden.",
    wirkungHeading: "Wirkungsbereiche",
    wirkungGroups: [
      {
        title: "Mentale Schwierigkeiten",
        items: [
          "Konzentrationsschwierigkeiten",
          "Lernschwierigkeiten",
          "Motivationsprobleme",
          "Leistungsabfall",
          "Energiemangel",
          "Blockierte Ressourcen",
          "Selbstwertprobleme",
        ],
      },
      {
        title: "Veränderungen / Lebensphasen",
        items: [
          "Schwangerschaft",
          "Kinderwunsch",
          "Wechseljahrbeschwerden",
          "Pubertät",
          "Trauer",
          "Herausfordernde Entwicklungsschritte",
          "Überfordernde Erlebnisse",
        ],
      },
      {
        title: "Körperliche Beschwerden",
        items: [
          "Schmerzen",
          "Muskuläre Verspannungen",
          "Haltungsprobleme",
          "Koordinationsstörungen",
          "Überbelastungen",
          "Gewichts- und Verdauungsprobleme",
        ],
      },
      {
        title: "Seelische Belastungen",
        items: [
          "Stressempfindungen, Ängste",
          "Erschöpfungszustände, Burnout",
          "Stimmungsschwankungen",
          "Mobbing",
          "Schlafbeschwerden",
          "Psychosomatische Beschwerden",
        ],
      },
    ],
    terminHeading: "Terminvereinbarung",
    termin:
      "Es besteht die Möglichkeit den Termin gleich selbst online zu buchen. Natürlich ist es auch möglich, mich telefonisch, per e-Mail oder WhatsApp/SMS zu kontaktieren.",
  },
  sport: {
    title: "SPORT-KINESIOLOGIE – DEINE QUELLE",
    h1: "SPORT-KINESIOLOGIE",
    sub: "Komplementärtherapie KT",
    leadTitle: "SPORTKINESIOLOGIE",
    lead:
      "Durch den Zusammenhang von Körper, Geist und Seele oder anders ausgedrückt die stetige Wechselwirkung zwischen Physis, Psyche und Biochemie ist nicht ausschliesslich die körperliche Form massgebend für die sportliche Leistung. Auch innerhalb der Sportkinesiologie werden die drei Ebenen miteinbezogen und ausbalanciert, was die Leistungsfähigkeit oder auch Regenerationsfähigkeit eines Sportlers massgeblich positiv beeinflusst.",
    bullets: [
      "Behandlung von körperlichen und geistigen Beschwerden sowie Optimierung des biochemischen, organischen und zellulären Ablaufs im Körper",
      "Trainingsoptimierung durch Schritt-für-Schritt-Optimierung einzelner Elemente aus dem entsprechenden Sportbereich",
      "Wettkampfvorbereitung in Form eines ganzheitlichen Stressmanagements",
      "Unterstützung beim Management von Blockaden und Krisen",
      "Fördern der Rekonvaleszens nach körperlicher Erschöpfung und/oder Verletzungen",
      "Vorbereitung auf Operationen",
      "Zusammenarbeit mit Trainern, Ärzten, Physiotherapeuten sowie weiteren gesundheitsfördernden Therapieformen",
    ],
  },
  yoga: {
    title: "YOGA – DEINE QUELLE",
    h1: "Hatha YOGA",
    subtitle: "Die Grundschule des Yoga.",
    intro:
      "Yoga beseitigt Störungen im physischen, emotionalen und mentalen Körper und hilft Wohlbefinden, Leichtigkeit, Ausdauer und Gesundheit wiederzugewinnen.",
    satyanandaTitle: "YOGA MIT INSPIRATION AUS DER SATYANANDA YOGA®-LEHRE",
    satyananda: [
      "Alle Yogalektionen sind inspiriert aus der Satyananda Yoga®-Lehre, einem integralen, von der Wissenschaft bestätigten Yogasystem. Es vereint traditionelle Techniken und Methoden, die von Swami Satyananda Saraswati erforscht, angepasst und genau beschrieben wurden, um sie den Menschen der heutigen Zeit zugänglich zu machen.",
      "Aus der Sicht von Satyananda Yoga® wird der Mensch als vielschichtiges Wesen betrachtet. In der Satyananda Yoga®-Lehre wurden Hatha Yoga, Raja Yoga, Karma Yoga, Bhakti Yoga und Jnana Yoga zu einem ganzheitlichen System vereint, um die ganze Persönlichkeit des Menschen anzusprechen. Es ist die Absicht, den Körper-Geist-Komplex in ein harmonisches Zusammenspiel zu bringen.",
      "Während einer Yogalektion werden verschiedene Techniken integriert. Asanas (Körperübungen) stärken den physischen Körper, machen ihn geschmeidig und bauen die Brücke zwischen der körperlichen und der mentalen Ebene. Pranayamas (Atemtechniken) erhöhen und stabilisieren die Lebensenergie (Prana/Chi), Tiefenentspannung und Meditation beruhigen und konzentrieren den Geist. Mit Mudras und Bandhas werden die Energieflüsse in bestimmte Richtungen gelenkt. Shatkarmas sind Reinigungstechniken für Körper und Energie. Als natürliche Folge des stetigen Übens wird inneres Gleichgewicht gefördert und persönliche Entwicklungsprozesse werden angeregt.",
    ],
    nidraTitle: "Die Integration von Satyananda Yoga Nidra® innerhalb der Yogalektion",
    nidra: [
      "Satyananda Yoga Nidra® (Tiefenentspannung), sowie eine Fülle von Meditationstechniken wurden von Swami Satyananda aus traditionellen Quellen abgeleitet, angepasst und präzise beschrieben um sie Yogaübenden der heutigen Zeit zugänglich zu machen.",
      "Moderne Entwicklung unseres Alltags führt dazu, dass wir konstant mit einer Vielzahl von Reizen, die unseren Geist überladen, überflutet werden. Es entsteht Anspannung und Nervosität auf körperlicher, mentaler und emotionaler Ebene. Die Folge davon sind fehlendes Gleichgewicht, schwache Konzentration und Ermüdungserscheinungen.",
      "Im Zustand von Satyananda Yoga Nidra® und Meditation wirken wir dem Chaos entgegen, indem wir die Wahrnehmung temporär von den äusseren Reizen abziehen und schrittweise nach innen lenken. Dies ermöglicht nicht nur vollkommene Entspannung und Erholung für Körper und Geist, sondern auch mentale Klarheit und tiefe Einsicht in unsere Innenwelt. Dadurch zeigen sich uns Wege auf, wie wir an uns arbeiten, neue Ziele definieren und vorhandenes Potenzial nutzen können.",
    ],
    kurseTitle: "AKTUELLE KURSE",
    kurseIntro: "Hatha YOGA mit Pratyahara-Meditation – 75/90 Minuten:",
    kurseZeiten: [
      "Montag: 9.30 – 11.00 (90 Min.)",
      "Montag: 18.15 – 19.30 (75 Min.)",
      "Montag 19.45 – 21.15 (90 Min.)",
      "Mittwoch: 18.00 – 19.30 (90 Min.)",
      "Mittwoch: 19.45 – 21.00 (75 Min.)",
    ],
    kurseHinweis:
      "Die Kurse werden im Studio wie auch online durchgeführt. Mindestteilnehmerzahl: 3 Personen Privatlektionen, Firmenworkshops/-kurse auf Anfrage Anmeldung unter info@deinequelle.com oder Telefon 076 413 80 50.",
    fuerWenTitle: "Hatha Yoga eignet sich für Jedermann!",
    fuerWen: [
      "Kinder und Jugendliche",
      "Studenten & Berufstätige",
      "Junge Eltern",
      "Ältere Personen",
      "Sportler",
      "ADS/ADHS-Kinder/Erwachsene",
      "Personen in physisch, psychisch und mental belastenden Situationen",
      "Personen mit Burn-out-Erkrankungen",
      "Personen mit psychosomatischen Erscheinungen",
      "Personen mit chronischen Erkrankungen wie z.B. Hypertonie, Asthma, Spannungskopfschmerzen",
      "Personen mit Übermüdung",
    ],
    nidraListeTitle: "Satyananda Yoga Nidra® hilft bei einer Vielzahl von Beschwerden!",
    nidraListe: [
      "Tiefenentspannung",
      "Burn-out -Therapie unterstützend",
      "Stressbewältigung",
      "Ausgleich von hormonalen Störungen",
      "Schwangerschaftsunterstützung",
      "Lernfähigkeitsverbesserung, konzentrationssteigernd",
      "Behebung von Störungen des körperlichen Gleichgewichts",
      "Behebung von Schlafstörungen",
      "Behebung von Angstzuständen",
      "Mentale Stärkung zur spirituellen Weiterentwicklung",
    ],
    preiseTitle: "PREISE",
    preise75: {
      title: "Lektionen à 75 Minuten",
      lines: [
        "Einzellektion: Fr. 34.00/Lektion",
        "Abo: Fr. 28.50/Lektion (Vordefinierte Blöcke)",
        "Online-Einzellektion: 30.00/Lektion",
        "Online-Abo: Fr. 25.00/Lektion (Vordefinierte Blöcke)",
      ],
    },
    preise90: {
      title: "Lektionen à 90 Minuten",
      lines: [
        "Einzellektion: Fr. 39.00/Lektion",
        "Abo: Fr. 33.50/Lektion (Vordefinierte Blöcke)",
        "Online-Einzellektion: 35.00/Lektion",
        "Online-Abo: Fr. 29.00/Lektion (Vordefinierte Blöcke)",
      ],
    },
    preiseFuss:
      "Kosten für Privatlektionen wie auch Firmenworkshops/-kurse auf Anfrage Workshops gemäss Ausschreibung",
    cssCoin:
      "Partner von CSS Coin Gerne akzeptiere ich auch CSS Coin als Zahlungsmittel .",
    shavasanaTitle: "SHAVASANA - ENTSPANNUNGSÜBUNGEN",
    shavasana: [
      "Shavasana Entspannungsübung (8 Minuten)",
      "Shavasana Entspannungsübung (15 Minuten)",
    ],
  },
  ueberMich: {
    title: "ÜBER MICH – DEINE QUELLE",
    h1: "HALLO, ICH BIN CLAUDIA",
    bioParas: [
      "Am 01.11.1972 wurde ich in Luzern als zweites von zwei Kindern geboren. Ich absolvierte die Primar- und anschliessend die Sekundarschule in Luzern und Adligenswil. Mit 19 Jahren schloss ich die Ausbildung als medizinische Praxisassistentin ab und arbeitete danach knappe zehn Jahre auf meinem erlernten Beruf. Eines meiner allerliebsten Hobbies ist bis heute das Tanzen. Meine Freizeit füllte ich mit Kursen im klassischen Ballett, Modern Dance, Hip Hop sowie Jazz Dance. Mit 20 Jahren absolvierte ich bei der AVON DANCE Academy ein Intensivjahr, welches ich dem Tanzen widmete. Danach kehrte ich wieder zu meinem angestammten Beruf zurück. Mit 26 Jahren heiratete ich und wurde in den Jahren 2001, 2002, 2008 sowie 2009 Mami von 4 Mädchen.",
      "Die körperlich wie auch emotional intensive Zeit mit der Familie veranlasste mich, eine in kurzer Zeit wirksame Entspannungstechnik zu finden. Das war mein Weg zu Satyananda Yoga Nidra® und Yoga welches mir zu viel Entspannung und auch innerer Ruhe verhalf und dies auch noch bis heute tut.",
      "«Yoga und Satyananda Yoga Nidra® hat meiner Person sowie meinem Leben so viel Kraft vermittelt, dass ich mich entschlossen habe, möglichst vielen Menschen diesen Weg zur inneren Ruhe zu zeigen, damit sie ihre eigene Quelle entdecken und für ihre Anliegen im Leben optimal nutzen können.»",
      "Schon während dem intensiven Familienalltag und meiner Zeit der Yogapraxis lernte ich die Komplementärtherapie kennen und deren Wirkungsbereich bewundern. Mir wurde klar, dass der Weg zu sich selbst manchmal auch eine intensivere Intervention bedeuten kann, um so die Selbstheilungskräfte über einen weiteren Weg zu aktivieren. Mit der komplementärtherapeutischen Technik der Kinesiologie habe ich eine wunderbare und meiner Person entsprechende Therapieform gefunden, mit der ich von Herzen gerne Menschen auf ihrem Weg persönlich wie auch gesundheitlich begleiten möchte.",
    ],
    ausbildungTitle: "MEINE AUSBILDUNG",
    ausbildung: {
      grund: [
        "Medizinische Praxisassistentin, 14.02.92",
        "Ernährung und Verpflegung, Bildungsgang Bäuerin, 22.09.11",
        "Ernährung und Verpflegung Modul 1, Bildungsgang Bäuerin, 21.03.21",
      ],
      yoga: [
        "Satyananda Yoga®-Ausbildung 2, Februar 2025 – September 2025",
        "Pawanmuktasana & Surya Namaskara, Juni 2019 – Februar 2020",
        "Pranayama & Pratyahara-Meditation, Oktober 2018 – Juli 2019",
        "Satyananda Yoga Nidra®, Dezember 2017 – September 2018",
        "3D Yogatrainer, 13.03. – 20.05.17",
        "Weiterbildung Satyananda Yoga Nidra®, Claudia Eva Reinig, 20.03.17",
        "Satyananda Yoga Nidra® Kursleiterin, 7. – 11.09.16",
      ],
      kinesiologie: [
        "Eidgenössisches Diplom Komplementärtherapie Fachrichtung Kinesiologie, April 2026",
        "Dipl. Komplementärtherapeutin Kinesiologie AKT, 29.11.2022",
        "My Kinesiologie Partnerin AKT, 07.07.21",
      ],
    },
  },
  infos: {
    title: "INFOS – DEINE QUELLE",
    h1: "INFORMATIONEN",
    sitzungTitle: "SITZUNGSINFORMATIONEN",
    sitzung:
      "Eine Erstbehandlung dauert in der Regel 90 Minuten, weitere Behandlungen 60-75 Minuten. Der Tarif beträgt 132 CHF pro 60 Minuten und wird der Sitzungsdauer in 5-Minuten-Einheiten angepasst.",
    krankenkasseTitle: "KRANKENKASSE",
    krankenkasse:
      "Meine kinesiologische Arbeit ist EMR-Zertifiziert. Die Abklärung über die Kostenübernahme durch die Krankenkasse liegt in der Pflicht jeder einzelnen Person selbst und ist bestenfalls im Voraus abzuklären.",
    absageTitle: "TERMINABSAGE",
    absage:
      "Eine Terminabsage ist kostenlos bis 24 Std. vor dem Termin möglich.",
    downloadsTitle: "DOWNLOADS",
    downloadsIntro: "AGB / Impressum / Schweigepflicht & Ethik-Richtlinien / Datenschutzerklärung",
    zahlungTitle: "ZAHLUNGSMÖGLICHKEITEN",
    zahlung: "Bar, TWINT oder per Rechnungsstellung",
  },
  documents: {
    agb: "/documents/AGB-Kine_Yoga.pdf",
    impressum: "/documents/Impressum-Kine_Yoga.pdf",
    ethik: "/documents/Ethik-Richtlinien.pdf",
    datenschutz: "/documents/Datenschutzerklaerung-DSE.pdf",
  },
} as const;

export type PagesData = typeof pages;
